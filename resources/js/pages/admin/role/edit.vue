            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h4 class="card-title">{{ $t("edit") + " " + $t("role") }}</h4>
                                    ...
                                    <div class="form-group mb-3">
                                        ...
                                        <div>
                                            <input v-model="roleForm.name"
                                                ...
                                                class="form-control" :placeholder="$t('enter_role_name')"
                                                autocomplete="off" />
                                            <has-error :form="roleForm" field="name"></has-error>
                                        </div>
                                    </div>
                                    <div class="form-group mb-3">
                                        <button :disabled="roleForm.busy" type="submit" class="btn btn-primary">
                                            {{ $t("update_role") }}
                                        </button>
                                    </div>
                                </div>
                                                ...
                                                <div class="d-inline-block">
                                                    <div>
                                                        <label class="form-check">
                                                            ...
                                                            <span class="form-check-label">{{ $t("all_permission") }}</span>
...
export default {
    created() {
        ...
        axios
            .get(`/api/roles/${this.$route.params.roleId}/edit`)
            .then(response => {
                ...
                if (role.permissions && role.permissions.length) {
                    role.permissions.forEach(element => {
                        this.roleForm.permission.push(element.id);
                    });
                }
            })
            .catch(error => {
    ...
    methods: {
        roleSave() {
            this.roleForm
                .put(`/api/roles/${this.$route.params.roleId}`)
                .then(response => {
    ...
    async mounted() {
        await this.hasPermisssion("role-edit");
