    ...
    <div class="col-md-6 col-xl-3">
        <div class="card user-card">
            <div class="card-body text-center">
                <div class="mb-3">
                    <img :src="student.user.image_url" alt="image" class="avatar avatar-xl avatar-rounded rounded"/>
                </div>
                <div class="card-title mb-1">{{ student.user.name }}</div>
                <div class="dots dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-end" :class="{ show: showDots }">
                        <router-link class="dropdown-item"
                            :to="{ name: 'user-student-edit', params: { id: student.id } }">
                            {{ $t("edit") }}
                        </router-link>
            ...
            <router-link :to="{ name: 'user-student-view', params: { id: student.id } }" href="#" class="card-btn">
                {{ $t("view_details") }}</router-link>
        </div>
        <DeleteModal :isShow="isDeleteModalShow" @close-modal="toggleDeleteModal" @delete-data="deleteStudent" />
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async deleteStudent() {
                try {
                    const response = await axios.delete(`/api/students/${this.student.id}`);
